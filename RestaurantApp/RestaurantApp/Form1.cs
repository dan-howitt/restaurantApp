...
namespace RestaurantApp
{
    public partial class Form1 : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            //set current user
            currentUser = new Account(txtRegUser.Text, txtRegPass.Text, role);

            //register the account - returns true if succesful
            if (currentUser.RegisterAccount(txtRegUser.Text, txtRegPass.Text, role) == true)
            {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            currentUser = new Account(txtLogUser.Text, txtLogPass.Text, "NEW");

            check = currentUser.CheckLogin(txtLogUser.Text, txtLogPass.Text);
        ...
        {
            //set temp Restaurant
            Restaurant tempRest = new Restaurant("NEW", "NEW", "NEW", "NEW", "NEW", 0);


            //get all Restaurants - check role first
            if (currentUser.GetRole() == "Customer")
            {
                restaurants = tempRest.GetAllRestaurants(0);
            }
            else 
            {
                restaurants = tempRest.GetAllRestaurants(currentUser.GetUserID());
            }
        ...
        private void LoadRestaurantDisplay(int index)
        {
            ...
            if(restaurants.Count != 0)
            {
               ...
               // String aRestaurant = restaurants[index].GetRestaurantDisplay();
                ...
                lblRestaurantName.Text = restaurants[index].GetRestaurantName();
                lblDescription.Text = restaurants[index].GetRestaurantDescription();
                lblAddress.Text = restaurants[index].GetRestaurantAddress();
            ...
            if (restaurants.Count <= 1 || index == restaurants.Count -1)
            {
                btnNext.Enabled = false;
            }
            else
            ...
            if (restaurants.Count <= 1 || index == 0)
            {
                btnPrevious.Enabled = false;
            }
            else
        ...
        {
            String aRestaurant = restaurants[activeRestaurant].GetRestaurantEdit();
            string[] aRest = aRestaurant.Split('#');
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if(isIDValid == false)
                {
                    SaveNewRestaurant(name, address, description, days, hours, tables, currentUser.GetUserID());
                }
                else
                {
        ...
        private void SaveNewRestaurant(string name, string address, string description, string days, string hours, int tables, int userid)
        {
            ...
            restaurants.Add(new Restaurant(name, address, description, days, hours, tables));

            //check if restaurant already exists
            if (restaurants[restaurants.Count - 1].CheckExists(name))
            {
            ...
            else
            {
                ...
                restaurants[restaurants.Count - 1].RegisterRestaurant(name, address, description, days, hours, tables, userid);
        ...
        private void UpdateRestaurant(int id, string name, string address, string description, string days, string hours, int tables)
        {
            ...
            //add permenant storage
            restaurants[0].UpdateRestaurant(id, name, address, description, days, hours, tables);
        ...
        private void btnDisplay_Click(object sender, EventArgs e)
        {
            ...
            //set temp booking
            Booking tempBook = new Booking(0, DateTime.Today, 0, 0);
            ...
            List<string> bookings = tempBook.GetBookings(restaurants[activeRestaurant].GetRestaurantID(), userID);
        ...
        private void btnBook_Click(object sender, EventArgs e)
        {
            ...
            lblBookCustName.Text = currentUser.GetName();
        ...
        private void GetAvailableTimes(string aDate)
        {
            ...
            //get existing bookings
            List<int> bookedTimes = tempBook.GetTimes(restaurants[activeRestaurant].GetRestaurantID(), aDate, restaurants[activeRestaurant].GetRestaurantTables());


            //get openingTimes
            string openingTimes = restaurants[activeRestaurant].GetRestaurantHours();
        ...
        private void btnSaveBooking_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                tempBook.BookTable(restaurants[activeRestaurant].GetRestaurantID(), dteBookDate.Text, currentUser.GetUserID(), (int)lstBookTimes.SelectedItem);
